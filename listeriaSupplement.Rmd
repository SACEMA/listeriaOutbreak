---
title: "Use of model-based projections during the 2017-2018 listeriosis outbreak in South Africa"
...
## Characterization of noise in observed case numbers

As described in the main text, therere an average of `r round(baseline,1)` weekly cases from Week 40 of 2017 through Week 8 of 2018. We fit Poisson and Negative Binomial error distributions to the case data from this timeframe using maximum likelihood and compared fits using Akaike's Information Criterion (AIC). Lower AIC values indicate better models. Observed variation around the mean value was better characterized by the Poisson model (Table \ref{tab:noise}), so we assumed Poisson noise around the expected value in the projections.

```{r noise, echo=FALSE}
tmp <- tibble(Model = c('Poisson', 'Negative Binomial'), AIC = c(AIC(pfit),AIC(nbfit)))
knitr::kable(tmp, digits = 2, caption = '\\label{tab:noise}Model fits for characterization of noise in case numbers.')
```
...
The following caveats were emphasized in the report:
- Note that we cannot confidently capture the tail of the distribution based on the data available.
- This time may actually be shorter, and the assumption is easily modified. 
- As data on the incubation period becomes available for the current outbreak in South Africa, we will use these data to update estimates that are likely to be more relevant for this outbreak, given the differences in background population and the source of exposure.
